...
public interface CliHandler {
   ...
   * @throws IllegalStateException if the {@link Environment} was required for a handler but not
   *                               present
   */
  static void handle(final OptionSet options, final Environment env,
      ...
      if (handler.canHandle(options)) {
        if (handler.requiresEnvironment() && !env.isAvailable()) {
          throw new IllegalStateException(
              "Handler requires environment but none is available" + handler);
        }
   ...
   * @return {@code false} if we want to stop processing reminder of handlers, otherwise {@code
   * true}
   */
   ...
   */
  default String getOrDefault(final OptionSet options, final String tag,
