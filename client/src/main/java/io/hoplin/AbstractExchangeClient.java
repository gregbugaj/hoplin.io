...
abstract class AbstractExchangeClient implements ExchangeClient {
    ...
    private static String DEFAULT_ERROR_EXCHANGE = "hoplin.error.exchange";
     ...
     */
    private void setupErrorHandlingExchange(final String exchangeName) {
    ...
    private void setupErrorHandlingQueue(final String queueName, final String exchangeName, final String bindingKey) {
        try {
            final boolean autoDelete = false;
            final String dlqQueueName = queueName +".error";
            final String dlqExchangeName = createDlqExchangeName(exchangeName);
            final AMQP.Queue.DeclareOk queueDeclare = client
                    .queueDeclare(dlqQueueName, true, false, autoDelete, Collections.emptyMap());

            client.queueBind(dlqQueueName, dlqExchangeName, bindingKey);
            log.info("Binding error queue [exchangeName, queueName, bindingKey, autoDelete] : {}, {}, {}, {}",
                    dlqExchangeName,
                    dlqQueueName,
                    bindingKey,
                    autoDelete
     ...
     * @return
     */
    private String createDlqExchangeName(String exchangeName) {
        // TODO : this should be controlled via a policy
        if (true) {
            return DEFAULT_ERROR_EXCHANGE;
        }
        final String name = (exchangeName == null || exchangeName.isEmpty()) ? DEFAULT_ERROR_EXCHANGE : exchangeName;
    ...
    <T> SubscriptionResult subscribe(final SubscriptionConfig config, final Class<T> clazz) {
        ...
        try {
            ...
            setupErrorHandlingQueue(queueName, exchangeName, bindingKey);
    ...
    void bind(final String type) {
        ...
        try {
            ...
            setupErrorHandlingExchange(exchangeName);
        } catch (final Exception e) {
